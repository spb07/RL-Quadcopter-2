...
class ReplayBuffer:
    ...
    def sample(self, batch_size=64):
        """Randomly sample a batch of experiences from memory. Option to upgrade to Prioritized Experience Replay if needed."""
...
class DDPG():
    ...
    def act(self, state, eps =1):
        ...
        return list(action + eps * self.noise.sample())  # add some noise for exploration
    
    def target_act(self, state): #best agent acting
        """Returns actions for given state(s) as per current policy."""
        state = np.reshape(state, [-1, self.state_size])
        #print("Reshaped state: ",state)
